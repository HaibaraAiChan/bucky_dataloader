...
class Trainer(object):
    def __init__(self, data, args, log_path):
        ...
        #     self.device)
        self.sage = SAGE(128, 128, 128, len(self.fanouts_train), torch.nn.functional.leaky_relu, 0.1).to(
            self.device)
    ...
    def run(self):
        ...
        return self.valid_accu[best_epoch], 0

    def train(self, epoch, optimizer, lr_scheduler):
        self.sage.train()
        for n_iter, ([input_nids, output_nids, blocks], labels) in enumerate(tqdm(self.train_node_loader, desc=f'train epoch {epoch}')):
            print('blocks ', blocks)
